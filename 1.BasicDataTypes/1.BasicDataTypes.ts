...
let someString: string = `some value for this string`; //string, number, boolean, object - other tsc types
...
['this is the string from the tuple and next elements will be the same as the definition of let tuple otherwise tsc will complain', 1, true, {simpleObjectFromTuple: 'yes this is tuple'}];
...
let exampleOfAnyType: any = `this strings type now can be changed from every next single line`;
...
const functionWithOptionalParameter = (name: string = 'name', age?: number): string => {
   if(age) {
...
const returnDataTypes = (numberOne: number, numberTwo: number): number => {
...
//---------------------------------------------------------------Intersection types------------------------------------//
//-----------------------------Definition - Intersection types  '&'--------------------//

interface Person {firstName: string }
interface Contact {phoneNumber: number }

const fullInfo: Person&Contact = { firstName: 'Test name 1', phoneNumber: 088832478903427908 };